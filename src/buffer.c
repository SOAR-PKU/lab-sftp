...
int ssh_buffer_prepend_data(struct ssh_buffer_struct *buffer, const void *data,
    ...
    memmove(buffer->data + len, buffer->data + buffer->pos,
            buffer->used - buffer->pos);
...
int ssh_buffer_add_buffer(struct ssh_buffer_struct *buffer,
    ...
    rc = ssh_buffer_add_data(buffer, ssh_buffer_get(source),
 ...
 * @return              The new size of the buffer.
 */
uint32_t ssh_buffer_pass_bytes_end(struct ssh_buffer_struct *buffer,
                                   uint32_t len) {
    if (buffer->used < len) {
        return 0;
    }
 ...
 * @internal
 *
 * @brief gets a 32 bits unsigned int out of the buffer. Adjusts the read
 * pointer.
 *
 * @param[in]  buffer   The buffer to read.
 *
 ...
 */
struct ssh_string_struct *ssh_buffer_get_ssh_string(
 ...
 */
int ssh_buffer_pack_va(struct ssh_buffer_struct *buffer, const char *format,
 ...
 * @brief Add multiple values in a buffer on a single function call
 * @param[in] buffer    The buffer to add to
 * @param[in] format    A format string of arguments. This string contains
 * single letters describing the order and type of arguments: 'b': uint8_t
 * (pushed in network byte order) 'w': uint16_t (pushed in network byte order)
 ...
 */
int ssh_buffer_unpack_va(struct ssh_buffer_struct *buffer, const char *format,
                         size_t argc, va_list ap) {
    int rc = SSH_ERROR;
 ...
 * @param[in] format    A format string of arguments. This string contains
 * single letters describing the order and type of arguments: 'b': uint8_t *
 * (pulled in network byte order) 'w': uint16_t * (pulled in network byte order)
